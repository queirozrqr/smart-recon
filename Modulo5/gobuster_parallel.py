...
def parallel():
    os.system('rm -rf /docker/data/teste/temp/gobuster_parallel.log')
    with open ('/docker/data/'+target+'/temp/gobuster_parallel.log','a') as file:
        for sis in dic_sistemas:
            file.write('python3 /docker/scripts/Modulo5_parse_gobuster.py '+target+' '+dic_sistemas[sis][0]+' '+dic_sistemas[sis][1]+' '+sis+' '+dic_sistemas[sis][3]+' '+str(dic_sistemas[sis][2])+'\n')
